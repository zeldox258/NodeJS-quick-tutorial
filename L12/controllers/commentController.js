...
const createComment = async (req, res) => {
   try {
      const { content, entryId, filePath, videoPath } = req.body;
      ...
      let commentObj = { content, userId: req.user.id, entryId, likes: [] };
      let mediaArray = [];
      if (filePath && Array.isArray(filePath)) {
         for (const path of filePath) {
            const result = await uploadController.uploadImage(path);
            mediaArray.push(result);
         }
      }

      if (videoPath && Array.isArray(videoPath)) {
         for (const path of videoPath) {
            const result = await uploadController.uploadVideo(path);
            mediaArray.push(result);
         }
      }

      commentObj.media = mediaArray;
